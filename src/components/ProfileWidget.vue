  ...
  <div class="profile-widget flex items-center z-10 relative">
    ...
    <div class="data flex-1 flex flex-col bg-[var(--grey-dark)] rounded-xl">
      <div class="top flex">
        ...
        <div
          class="top-content flex items-center gap-4 justify-between p-2 flex-1"
        >
          ...
          >
            <span class="text-base text-gray-200">{{
              battleLocale?.["button_boost"] || "Boost"
            }}</span>
          </Button>
        </div>
      </div>
      ...
      <!-- stats -->
      <div
        class="bottom grid grid-cols-[3fr_2fr_2fr] bg-[var(--grey-light)] py-1 rounded-xl pl-[55px]"
      >
        <!-- bolts -->
        <div
          class="bolts flex-1 flex items-center gap-1 relative justify-center"
        >
          <img class="h-4" :src="getAsset('bolt')" />
          <span
            data-locale="profile.tooltip_bolts"
            class="exo-black text-[var(--accent-color)]"
            @click="onTooltip"
            >{{ shortenNumber(profileData?.["bolts"]) ?? 0 }}</span
          >
          <div
            class="separator h-[10px] w-[1px] bg-gray-500 absolute -right-1 mt-[2px]"
          ></div>
        </div>

        <!-- nuts -->
        <div
          class="nuts flex-1 flex items-center gap-1 relative justify-center"
        >
          <img class="h-4" :src="getAsset('nut')" />
          <span
            data-locale="profile.tooltip_nuts"
            class="exo-black"
            @click="onTooltip"
            >{{ profileData?.["nuts"] ?? 0 }}</span
          >
          <div
            class="separator h-[10px] w-[1px] bg-gray-500 absolute -right-1 mt-[2px]"
          ></div>
        ...
        <div class="energy flex items-center gap-1 justify-center">
          ...
          <span
            data-locale="battle.tooltip_energy"
            class="exo-black"
            @click="onTooltip"
            >{{ battleData?.["energy"] ?? 0 }}</span
          >
        </div>
      </div>
    </div>
...
const { profile: profileData, battles: battleData } = storeToRefs(
  store.dataStore
);
const { profile: profileLocale, battles: battleLocale } = storeToRefs(
  store.localeStore
);
