      ...
      >
        <div class="top flex gap-4">
          ...
          <div class="text flex flex-col">
            ...
            <span class="fira-condensed text-sm inline-svg svg-top-margin my-1" v-html="textWithSpecialSymbols(notification.subtitle)"></span>
          </div>
        </div>

        <div class="btns flex justify-end gap-8 px-2">
          <Button v-if="!notification.buttons?.left?.hidden" class="text-[var(--red-color)]" :data="notification.buttons.left" :defaultAction="hideNotification" unstyled></Button>
          <Button
            v-if="!notification.buttons?.right?.hidden"
            class="text-[var(--green-color)]"
