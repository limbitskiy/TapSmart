<template>
  <div class="data-content flex flex-col gap-2 items-center text-center mb-4">
    ...
    <span
      class="text-lg px-[10vw] text-gray-300"
      v-html="modal.subtitle"
    ></span>
    <div class="btns flex justify-end gap-4 mt-4">
      <Button
        v-if="!modal.buttons?.left?.hidden"
        class="!py-1 bg-[var(--red-color)]"
        :data="modal.buttons.left"
        :defaultAction="hideModal"
      />
      <Button
        v-if="!modal.buttons?.right?.hidden"
        class="!py-1 bg-[var(--green-color)]"
        :data="modal.buttons.right"
        :defaultAction="hideModal"
      />
    </div>
  </div>
