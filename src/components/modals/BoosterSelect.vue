  ...
  <div id="booster-select" class="flex-1 py-1 flex flex-col gap-4">
    <div class="pill-header flex items-center justify-between">
      <span class="bg-pill-title">{{
        locale?.["booster_select_title"] || "Challenge"
      }}</span>
      ...
      <Pill class="!py-2 rounded-[10px]">
        ...
        <div class="rewards flex items-center justify-center">
          <div class="mult flex gap-4 items-baseline justify-between">
            <div class="label">
              <span class="">
                {{ locale?.["multiplier_title"] || "Multiplier:" }}</span
              >
            </div>
            <div class="value flex gap-1 items-baseline">
              ...
              <span class="text-lg exo-black">
                {{ data?.["challenge_multiplicator"] || 0 }}</span
              >
            </div>
          </div>
        </div>
    ...
    <div class="boosters flex flex-col gap-2 flex-1">
      <span class="fira-bold text-lg">{{
        locale?.["available_boosters"] || "Available boosters"
      }}</span>
      <div class="boosters-content">
        ...
        <Pill
          ...
          id="extra-mistake"
          :class="{
            'outline outline-2 outline-[var(--accent-color)]':
              pickedBonuses['mistake'],
          }"
        >
          <div class="start flex gap-3">
            ...
            <div class="text flex flex-col">
              <span class="fira-semibold">{{
                locale?.["extra_mistake_title"] || "Extra mistake"
              }}</span>
          ...
          <div class="end flex justify-end">
            ...
            >
              <span class="exo-bold !text-base !text-white">{{
                locale?.["enable_boost"] || "Enable"
              }}</span>
            </Button>
            <Button
              v-else
            ...
            >
              ...
              <span class="exo-bold !text-base !text-white">{{
                data.boosters?.extra_mistake.price
              }}</span>
            </Button>
          </div>
        </Pill>
        ...
        >
          <div class="start flex gap-3">
            <div class="icon mt-1">
              ...
              >
                ...
                />
                <path
                  d="M27.885 30L19.5 21.615V7.5H22.5V20.37L30 27.885L27.885 30Z"
                  fill="#519A58"
                />
              </svg>
            </div>
            <div class="text flex flex-col">
              <span class="fira-semibold">{{
                locale?.["extra_battle_time_title"] || "Extra time"
              }}</span>
              <p class="text-gray-300">
          ...
          <div class="end flex justify-end">
            ...
            >
              <img class="h-4 scale-125" :src="getAsset('nut')" />
              <span class="exo-bold !text-base !text-white">{{
                data.boosters?.extra_time.price
              }}</span>
            </Button>
          </div>
        </Pill>
      ...
      >
        <div class="flex justify-center gap-1">
          <span class="text-xl leading-4">{{
            locale?.["button_booster_select"] || "Challenge"
          }}</span>
        </div>
      </Button>
      <Button
        class=""
        :class="onlineFriends.length ? '' : '!bg-black !text-white'"
        :badge="onlineFriends"
        @click="onFriendsOnly"
      >
        <div class="flex gap-1 justify-center">
          <span class="">{{
            locale?.["button_booster_friends"] || "Challenge friends"
          }}</span>
