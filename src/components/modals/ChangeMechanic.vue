  ...
  <div id="change-mechanic" class="flex-1 flex flex-col gap-4 py-1">
    <div class="pill-header flex items-center justify-between">
      <span class="bg-pill-title">{{
        locale?.["select_mechanic_title"] || "Select mechanic"
      }}</span>
    </div>

    <div
      class="pill-grid grid grid-cols-2 gap-4 p-[2px] flex-1 overflow-y-scroll"
    >
      <MechanicCard
        v-for="mech in battleMechanics"
        :key="mech!.id"
        :propData="mech!"
        @select="onMechSelect"
      >
...
const { getMechanicName, changeMechanic, stopBreakpoint, startBreakpoint } =
  dataStore.battles;
