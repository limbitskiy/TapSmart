<template>
  <div class="4-answers-battle flex-1 flex flex-col">
    ...
    >
      <div
        ref="el"
        class="question-text flex flex-col gap-2 items-center text-center w-full px-8 py-4 overflow-x-hidden text-ellipsis"
      >
        <span
          class="fira-condensed-black"
          style="font-size: clamp(30px, 9vw, 46px)"
          >{{ currentTask?.task?.question || "Question not found" }}</span
        >
      </div>
    </div>
    ...
    >
      ...
      <div
        class="answer-buttons grid w-full grid-cols-2 grid-rows-2 gap-4 px-4 leading-5"
      >
        <Button
          ...
          @click="
            (event) =>
              handleAnswer(currentTask.task.variants[0], currentTask, event)
          "
          >{{ currentTask.task.variants[0] }}</Button
        >
        <Button
          class="four-answer-btn"
          @touchstart="btnTouchstart"
          ...
          @click="
            (event) =>
              handleAnswer(currentTask.task.variants[1], currentTask, event)
          "
          >{{ currentTask.task.variants[1] }}</Button
        >
        <Button
          class="four-answer-btn"
          @touchstart="btnTouchstart"
          ...
          @click="
            (event) =>
              handleAnswer(currentTask.task.variants[2], currentTask, event)
          "
          >{{ currentTask.task.variants[2] }}</Button
        >
        <Button
          class="four-answer-btn"
          @touchstart="btnTouchstart"
          ...
          @click="
            (event) =>
              handleAnswer(currentTask.task.variants[3], currentTask, event)
          "
          >{{ currentTask.task.variants[3] }}</Button
        >
      </div>
    </div>
  </div>
