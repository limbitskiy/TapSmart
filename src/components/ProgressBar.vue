<template>
  <div class="progressbar flex flex-col gap-1">
      ...
      >
        ...
        >
          ...
          <circle
            ...
            cy="5.99632"
            r="4.6697"
            transform="rotate(180 6.00022 5.99632)"
            :fill="getMarkerColor(enemy.id)"
          />
        </svg>
      </div>
    ...
    <div
      ref="gaugeRef"
      class="gauge h-2 bg-[var(--grey-light)] rounded-full w-full"
    >
      <div
        ref="progressRef"
  ...
  <div class="debug flex flex-1 items-center justify-between">
    <div class="player flex flex-col">
      <span class="text-sm"
        >Player score: {{ Math.round(computedPlayer?.score) }}</span
      >
      <span class="text-sm"
        >Player progress: {{ Math.round(computedPlayer?.progress) }}</span
      >
    </div>
    <div class="enemy flex flex-col">
      <span class="text-sm"
        >Enemy score: {{ Math.round(computedEnemies[0]?.score) }}</span
      >
      <span class="text-sm"
        >Enemy progress: {{ Math.round(computedEnemies[0]?.progress) }}</span
      >
    </div>
  </div>
</template>
...
const { data, playerProgress, playerPositions } = storeToRefs(
  dataStore.battles
);
...
watch(
  ...
  () => {
    ...
    players.forEach((player) => {
      const playerPosition = playerPositions.value.find(
      ...
      playerPosition.progress +=
        ((player.score - playerPosition.score) /
          (highestScore - playerPosition.score)) *
        (secId - playerPosition.progress);
      playerPosition.score = player.score;
    });
  },
  {
...
const computedPlayer = computed(() =>
  playerPositions.value.find((position) => position?.isPlayer)
);

const computedPlayerProgress = computed(() => {
  const coef =
    gaugeRef.value?.getBoundingClientRect()?.width /
    (data?.value?.["battle_duration"] / 1000);
  if (!coef || !computedPlayer.value) return 0;
  return computedPlayer.value?.progress * coef;
});
const computedEnemies = computed(() =>
  playerPositions.value.filter((player) => !player?.isPlayer)
);
