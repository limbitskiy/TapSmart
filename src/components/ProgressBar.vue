<template>
  <div class="progressbar flex flex-col gap-1">
    <div class="enemy-markers relative h-4">
      <div v-for="enemy in computedEnemies" :key="enemy.id" class="marker absolute" :style="{ transform: `translateX(${(enemy?.progress ?? 0) - 6}px)`, transition: '1s linear' }">
    ...
    <div class="player-marker mt-2">
      <div class="marker w-[14px]" :style="{ transform: `translateX(${(computedPlayer?.progress ?? 0) - 7}px)`, transition: '1s linear' }">
  ...
  (players) => {
    ...
    players.forEach((player) => {
      ...
      if (player.score === highestScore) {
        const progressBarRect = progressRef.value?.getBoundingClientRect();

        playerPosition.progress = progressBarRect.width;
        playerPosition.score = player.score;
        return;
      }

      playerPosition.progress += (player.score - playerPosition.score) / (highestScore / resId);
      playerPosition.score = player.score;
    });
  },
  {
