<template>
  <div class="waiting-modal flex flex-col gap-4">
    <div class="header flex flex-col gap-4">
      ...
      <Pill color="light">
        <CountdownTimer
          class="exo-bold text-xl"
          :initValue="data['waiting_timer']"
        />
      </Pill>
    </div>
      ...
      <li v-for="player in data['players_waiting'] || []" :key="player.id">
        <Pill color="light">
          <div class="player-data flex gap-2 items-center">
            <span class="league exo-black">{{ player.league }}</span>
            <div class="player-name relative">
              <span class="fira-bold max-w-1/2 text-ellipsis">{{
                player.name
              }}</span>
              <div class="friend-badge absolute top-0 left-full ml-2">
                <svg
                  width="13"
                  height="9"
                  viewBox="0 0 13 9"
...
// const friends = computed(() => [{ id: 0, name: "Jack" }]);
</script>
