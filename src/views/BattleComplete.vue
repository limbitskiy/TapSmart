<template>
  <div
    id="battle-complete"
    class="flex-1 flex p-4 max-h-dvh flex-col relative overflow-hidden"
  >
    <!-- bg pattern -->
          ...
          <Pill class="!py-1">
            <div class="content flex flex-col items-center justify-between">
              <div class="place-icon-text flex gap-2 items-center">
                ...
                />
                <img
                  v-else-if="data?.['battle_complete_place'] === 2"
                  class="h-10"
                  :src="getAsset('2nd_place')"
                />
                <img
                  v-else-if="data?.['battle_complete_place'] === 3"
                  class="h-10"
                  :src="getAsset('3rd_place')"
                />
                <span v-else class="fira-bold text-2xl">{{
                  data?.["battle_complete_place"] || 0
                }}</span>
              </div>
        ...
        <div v-if="data?.['error_data']?.length" class="errors">
          <Pill class="!py-2 flex flex-col gap-2">
            <div class="stat flex gap-4 items-center justify-between">
              <span class="fira-condensed text-lg">{{
                locale?.["error_data_title"] || "Errors"
              }}</span>
              <span class="text-xl exo-black text-red-600">{{
      ...
      <div class="ad-btns w-full flex gap-4 mt-8">
        ...
        >
          <div class="flex flex-col gap-1 items-center">
            <div class="flex gap-1 items-baseline">
              <span class="text-base leading-4">{{
                getBtnTextArr(locale?.["button_claim_with_ton"])?.[0] ?? 0
              }}</span>
              <img class="h-4" :src="getAsset('bolt-dark')" />
              <span>{{ data?.["battle_complete_bolts_with_ton"] }}</span>
            </div>
            <span class="text-sm fira-regular !leading-3">{{
              getBtnTextArr(locale?.["button_claim_with_ton"])?.[1] ?? 0
            }}</span>
          </div>
        </Button>
      </div>
    ...
    <Transition name="fade-800">
      <div
        ...
        :style="`background: center/cover no-repeat url(${getAsset(
          'rays_purple'
        )})`"
      ></div>
    </Transition>
    ...
    <Transition name="closeup-score">
      <div
        v-if="isCloseup"
        class="place-icon-text-cnt absolute inset-0 grid place-items-center z-[25]"
      >
        <div
          class="place-icon-text flex flex-col gap-1 items-center justify-center w-full"
        >
          <span class="fira-condensed-bold text-4xl text-[var(--accent-color)]"
            >{{ locale?.["battle_complete_modal_title"] || "Congrats!" }}
          </span>
          <div class="place flex flex-col gap-6 items-center">
            <span class="fira-semibold text-2xl">{{
              locale?.["battle_complete_modal_subtitle"] || "Your place is:"
            }}</span>
            <img
              v-if="data?.['battle_complete_place'] === 1"
              class="h-12 scale-[2]"
