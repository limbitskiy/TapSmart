<template>
  <div id="battle-complete" class="flex-1 flex p-4 flex-col h-dvh overflow-hidden relative">
    <template v-if="isCloseupAnimation">
      <Transition name="fade-800" mode="out-in" appear>
        <div
          v-if="isBackdrop"
          class="backdrop absolute -left-[500px] -right-[500px] -top-[500px] -bottom-[500px] z-[99] spinning-bg"
          :style="`background: center/cover url(${getAsset('rays_green')})`"
        ></div>
      </Transition>
...
setTimeout(() => {
  isCloseupAnimation.value = false;
}, 4000);
