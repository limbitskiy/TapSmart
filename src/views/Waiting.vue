  ...
  <div id="waiting-cnt" class="flex flex-1 p-4">
    <BackgroundPill class="gap-4 mt-[10dvh] relative">
      <div class="header flex flex-col gap-4">
        ...
        <!-- subtitle -->
        <span class="">{{
          locale?.["waiting_subtitle"] || "Waiting subtitle"
        }}</span>
        ...
        <Pill>
          <div class="timer flex items-center gap-2 justify-center">
            ...
            >
              ...
              />
              <path
                d="M16.5982 17.8557L11.6072 12.8647V4.46289H13.3929V12.1236L17.8572 16.5968L16.5982 17.8557Z"
                fill="#C0C0C0"
              />
            </svg>

            <span class="exo-black text-3xl mr-[38px]">{{
              formattedTime(data?.["waiting_timer"]) ?? "00:00"
            }}</span>
          </div>
        </Pill>
      </div>
        ...
        <li v-for="player in data?.['players_waiting'] || []" :key="player.id">
          <Pill class="flex flex-col !py-2">
            ...
            <div class="other-info flex gap-2 items-center">
              <div class="league">
                <span class="league exo-bold"
                  >L: {{ player?.league || 0 }}</span
                >
              </div>
              ...
              <div class="bolts flex items-center gap-1">
                ...
                <span class="exo-black text-[var(--accent-color)]">{{
                  player?.bolts || 0
                }}</span>
              </div>
            </div>
      ...
      <div
        v-if="challengeProps.friends_only"
        class="btns w-full flex gap-4 mt-8"
      >
        <Button
          class="flex-1 py-3 !px-0"
          activeColor="#525252"
          @click="onInviteFriend"
          :dark="data?.['players_waiting']?.length > 1"
        >
          <span class="text-lg leading-4">{{
            locale?.["button_waiting_invite"] || "Invite"
          }}</span>
        </Button>
        <Button
          class="flex-1 py-3 !px-0"
...
const { startBreakpoint, stopBreakpoint, decreaseWaitingTimer } =
  store.battleStore;
