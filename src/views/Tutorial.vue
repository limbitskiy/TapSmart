  ...
  <div class="tutorial-cnt flex-1 flex flex-col p-4">
    <div class="tutorial-header py-2 flex items-center justify-between absolute top-2 w-[calc(100vw-2rem)] z-20">
      <div class="tutorial-header__start">
        <span class="font-medium text-[var(--accent-color)]">{{ data.store.slides[currentSlide].topic }}</span>
      </div>
      <div class="tutorial-header__end">
        <button class="border border-gray-400 px-6 py-1 rounded-lg" @click="onSkip">{{ getLocale("button_skip") }}</button>
      </div>
    </div>
    <div class="tutorial-body flex-1 flex mt-10">
      <Transition name="fade" mode="out-in">
        <TutorialSlide :data="data.store.slides[currentSlide]" :key="data.store.slides[currentSlide].id" />
      </Transition>
    </div>
...
const { getLocale } = useStoreHelpers("tutorial");
...
const isLastSlide = computed(() => !data.value.store.slides[currentSlide.value + 1]);
