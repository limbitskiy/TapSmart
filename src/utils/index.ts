...
export const shortenNumber = (num: number) => {
  if (num <= 999999) {
    ...
    return showFormattedNumber(num.toString().slice(0, -3)) + "K";
  }
};
...
// export const getSpecialSymbol = (symbolName: string) =>
//   specialSymbols[symbolName];

export const replaceWithSpecialSymbols = (text: string) => {
  let result = text;

  Object.keys(specialSymbols).forEach((symbol) => {
    result = result.replace(new RegExp(symbol), getSpecialSymbol(symbol));
  });
